...
**تسجيل الدخول (`/api/auth/login`)** إذا وُجد في البيئة **`ADMIN_EMAIL` و`ADMIN_PASSWORD` معًا** فيصبح الدخول **من البيئة فقط** (لا يُستخدم حساب Mongo لنفس البريد). بعد تغيير القيم **أعد تشغيل** خدمة التطبيق. يُستحسن أن يطابق البريد ما تكتبه في نموذج الدخول (يُطبَّع تلقائيًا: أحرف صغيرة وبدون مسافات زائدة في المتغير).
...
- في الإنتاج، **`/blog`** يُعاد بناء `index.html` من الخادم مع **`canonical` و`og:url` يشيران إلى `/blog`** (وليس الصفحة الرئيسية)، لتفادي تقرير Search Console مثل «صفحة بديلة تتضمن علامة أساسية مناسبة» بسبب HTML الأول قبل تنفيذ JavaScript. كذلك **`/index.html`** يُعاد توجيهه بـ **301** إلى **`/`**، و**`/blog/`** إلى **`/blog`** لتوحيد شكل الرابط.
