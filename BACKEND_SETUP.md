...
يُنشئ مستخدمًا من `ADMIN_EMAIL` / `ADMIN_PASSWORD` في البيئة. **إن وُجد مستخدم بنفس البريد**، يُحدَّث **hash كلمة المرور** من `ADMIN_PASSWORD` الحالية (مفيد بعد تغيير السرّ على الاستضافة). تسجيل الدخول في التطبيق يقرأ **قاعدة البيانات** وليس المتغيرات في كل طلب — تغيير البيئة وحده دون تشغيل هذا الأمر أو تحديث المستخدم لا يغيّر من يستطيع الدخول.

**ملاحظة:** ملف `.env.example` للتوثيق فقط؛ الاستضافة لا تقرأه. الاعتماد على متغيرات البيئة في لوحة النشر أو ملف `.env` على الخادم.
